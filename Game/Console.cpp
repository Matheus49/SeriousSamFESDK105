...
void CGame::ConsoleRender(CDrawPort *pdp)
{
  ...
  LCDRenderGrid();
  LCDRenderClouds2();
  dpConsole.DrawLine( 0, pixSizeJ-1, pixSizeI, pixSizeJ-1, LCDFadedColor(C_GREEN|255));
...
void CGame::ConsolePrintLastLines(CDrawPort *pdp)
{
  ...
  for( INDEX iLine=0; iLine<ctLines; iLine++) {
    ...
    pdp->PutText( strLine, 0, pixCharHeight*(ctLines-iLine-1), C_GREEN|255);
  }
}
